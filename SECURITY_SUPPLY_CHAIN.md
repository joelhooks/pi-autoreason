...
This repo treats dependency, workflow, and source/skill changes as security-sensitive.
...
- Prefer npm Trusted Publishing/OIDC/provenance over long-lived `NPM_TOKEN` secrets if this package is published.
- Require review for `.github/workflows/`, `.github/actions/`, package manifests, lockfiles, `.npmrc`, `src/`, `skills/`, and release config.
...
If a compromised package install ran: assume local/user secrets are burned, revoke npm/GitHub/cloud tokens, rotate deploy secrets, compare published artifacts against source, and rebuild the machine instead of surgical cleanup theater.
